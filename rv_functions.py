...
class Star():
    ...
    def plot(self):
        ...
        ax.set_xlabel('Time - 2450000 (Julian Days)',fontsize=15)
    ...
    def recover_params(self,*popt):
        ...
        mp *= (9.543e-4); mp /= 0.00314558 #mp -> Earth mass
        p = 2*np.pi/n
...
def plot_data(star,t_fit,vr_fit,residuals,popt,pcov):
    ...
    legstr = r'M_p>{:.3f} +\- {:.3f} M_e, e={:.3f} +\- {:.2f}, P={:.2f} +\- {:.2f} days'.format(mp,mp_err,e,e_err,p,p_err)
    ...
    ax.set_title(star.name + ', M = {:.2f} Msun'.format(star.mass),fontsize=15)
    ...
    axr.grid()
    axr.minorticks_on()
    axr.set_ylabel('O-C',fontsize=15)
    axr.set_xlabel('Time -2450000 (Julian Days)',fontsize=15)
